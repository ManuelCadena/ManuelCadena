<h1 align="center">Manuel Cadena</h1>
<p align="center">
  <em>PhD-level researcher & multidisciplinary builder</em><br/>
  AI · LLM Systems · Neuroscience · Corporate Finance · Law · AgriTech · HealthTech
</p>

<p align="center">
  <a href="https://mykaira.ai">mykaira.ai</a> ·
  <a href="mailto:manuelcadena@mac.com">manuelcadena@mac.com</a> ·
  Sandy, UT / México
</p>

---

### About me

I am a senior researcher operating at the intersection of **artificial intelligence, neuroscience,real estate, finance, and law**. I design and ship production-grade AI systems. I hold a **Master in Liberal Arts in Finance (ALM)** from **Harvard ** and an **MBA** from **IPADE Business School**.
My work spans multiple verticals — each one its own GitHub Organization:

| Vertical | Organization | Flagship |
|---|---|---|
| 🌱 **AgriTech** | `citrusmax-ai` | CitrusMax — Persian Lime yield maximization platform |
 🏠 **Real Estate** | `chatten-ventures` | Chatten House Funnel + LeadForge AI |
| 🪞 **Logos / Wellness** | `logos-equation` | Kaira — emotional wellness companion |
| 🍳 **FoodTech** | `chefmanny-foodtech` | ChefManny |
| 🏥 **HealthTech** | `hmg-health` | HMG CRM |
| 🤖 **Personal AI** | `chatita-labs` | Chatita — personal super-agent |
| 🎓 **Education** | `cadena-harvard` | ALM Finance · MBA · continuing education |
| 🔬 **Research** | `cadena-research` | Active inference · FEP · Bioelectricity |

---

### Research interests

- **Active Inference & the Free Energy Principle** (Friston framework)
- **Bioelectricity & biological computing**
- **LLM agents, tool-use & autonomous systems**
- **International migration law & corporate governance**

---

### Tech stack

```text
Languages   TypeScript · Python · R · SQL · Shell · KiCad
AI / ML     PyTorch · LangChain · LlamaIndex · OpenAI · Anthropic · Perplexity
Backend     Node.js · FastAPI · Supabase · PostgreSQL · GCP · Vercel
Frontend    React · Next.js · React Native · Tailwind
DevOps      GitHub Actions · Docker · Turborepo · pnpm · uv
Research    Stan · brms · tidyverse · Quarto · LaTeX
Hardware    PCB design · microcontrollers · sensor systems
```

---

### Currently

- 🔭 Building **CitrusMax AI** — modular AgriTech platform for Finca Citrícola La Luz
- 🧠 Researching **active inference** applied to autonomous AI agents
- ✍️ Writing on biocomputing, LLM safety, and agentic AI

---

<p align="center"><em>“The map is not the territory — but a well-engineered map ships value.”</em></p>
